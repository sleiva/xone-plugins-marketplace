...
description: "Desarrollo XOne: XML .xne, JS del runtime, CSS, datos, dispositivo, fundamentos. UI/.xne: colecciones, props/tipos, groups, frames, contents, asfilter, combos mapcol/mapfld, mapas, kanban, chips, layouts, inherits, include-layout, eventos XML, permisos, pantallas vacías, errores estructurales. JS y functions.js: self, selfDataColl, ui, appData, err, user, getControl, métodos de controles, singletons, creables, callbacks, lock/unlock, startBrowse/endBrowse, objeto ai. CSS: default.css y variantes, clases, selectores coll/prop:TYPE/group/frame, unidades p/%, colores #AARRGGBB, extends/@extend, :root/var(), calc(), @import, temas light/dark, animaciones, estilos dinámicos. Datos: SQL ##PREF##, SqlManager, macros globales, $http TLS/pinning/mTLS, futures, OAuth2, réplica ROWID, crypto, mocks HTTP. Dispositivo: GPS/geolocalización, cámara, foto/vídeo, escaneo QR/barcode, firma DR, permisos runtime, biometría, Bluetooth, impresión, NFC, DNI electrónico, WifiManager, FileManager, mock/device.json. Fundamentos: app.xml, app.ini, mappings.xne, carpetas bd/icons/files/fonts, macros del sistema, códigos de error, Splash→Login→EntradaApp→Menu, convenciones de nombres, sintaxis JS soportada."
---
