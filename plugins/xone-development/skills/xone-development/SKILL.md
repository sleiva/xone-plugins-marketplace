...
description: Desarrollo XOne, todas las áreas. UI/.xne: colecciones, props/tipos, groups, frames, contents, asfilter, combos mapcol/mapfld, mapas, kanban, layouts, inherits, eventos XML, permisos, pantallas vacías, errores estructurales. JS y functions.js: self, selfDataColl, ui, appData, getControl, controles, singletons/creables, lock/unlock, startBrowse/endBrowse, objeto ai. CSS: default.css y variantes, selectores coll/prop/group/frame, unidades p/%, colores #AARRGGBB, extends/@extend, var()/calc(), temas light/dark, animaciones. Datos: SQL ##PREF##, SqlManager, macros globales, $http TLS/pinning/mTLS, futures, OAuth2, réplica ROWID, crypto, mocks. Dispositivo: GPS/geolocalización, cámara, foto/vídeo, QR/barcode, firma DR, permisos runtime, biometría, Bluetooth, impresión, NFC, DNIe, WifiManager, FileManager, mock/device.json. Fundamentos: app.xml, app.ini, mappings.xne, carpetas bd/icons/files/fonts, macros, códigos de error, Splash→Login→EntradaApp→Menu, convenciones de nombres, sintaxis JavaScript soportada.
---
