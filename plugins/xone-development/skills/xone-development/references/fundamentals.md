...
Uso típico: scaffolding visual compartido (header/footer/navegación) definido una sola vez en una coll `special="true"` y reutilizado por todas las pantallas que lo necesiten. Soporta cadenas (A → B → C) pero NO herencia multiple.
...
| `crypto`, `clipboard`, `deviceInfo`, `systemSettings`, `packageManager`, `biometricsManager`, … | Singletons XOne. |
...
> Para el caso especial de abrir directamente la **lista** de una coll (no su EditView) → `ui.openMenu("Coll", mask, 0)`.
...
> **Forma corta:** `ui.openEditView("DetalleTarea")` crea un objeto vacío y lo abre en una sola llamada — útil cuando no hay que pre-rellenar nada.
