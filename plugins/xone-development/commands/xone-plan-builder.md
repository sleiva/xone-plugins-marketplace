...
1. **Carga la skill `xone-plan-builder`** desde `plugins/xone-development/skills/xone-plan-builder/SKILL.md` y síguela estrictamente. Carga también `xone-development` para conocer las dependencias naturales entre colls, pantallas, eventos y estilo.
...
7. **Escribe TASKS.md.** Usa el formato de `references/TASKS-FORMAT.md`. Decisiones, no código. Nombres consistentes con el spec (MAYÚSCULAS campos, PascalCase colls, MAP_ UI temporal).
