...
- **El corpus de referencia vuelve al repositorio, y solo `v2`.** El commit que retiró las copias planas de `xone/` afirmó que el material seguía en el repo bajo `xone/v1/` y `xone/v2/`, y era falso: `/xone/` está en `.gitignore` desde 0.10.0, así que esas dos carpetas **nunca estuvieron versionadas** y las planas eran la única copia que viajaba. Comprobado clonando: `xone/` no llegaba a existir, 0 ficheros frente a 79 en la máquina del autor. Ahora se versiona **`xone/v2`** —la versión viva, 39 ficheros— con `/xone/*` + `!/xone/v2/`, porque git no puede re-incluir algo dentro de un directorio excluido. `v1` y los comparadores entre versiones se quedan fuera a propósito. Las **70 citas `> Fuente:`** de 69 ficheros de referencia pasan de `xone/…` a `xone/v2/…` y **las 23 rutas distintas resuelven**, comprobadas una a una.
...
- ~~El corpus de `xone/` no está versionado.~~ **RESUELTO en esta misma versión**: ver «Corregido».
...
- El fichero lleva escrita su regla de admisión: una entrada entra solo si se ha confirmado funcionando, anotando la plataforma. Lo no confirmado no se escribe, y la página **no enumera la API de vistas de Android ni de iOS** — esa lista no pertenece a este repositorio. Diseño en `docs/superpowers/specs/2026-08-07-metodos-nativos-de-la-vista-design.md`.
