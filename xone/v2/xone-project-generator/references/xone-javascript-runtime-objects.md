...
Catálogo completo con ejemplos por área en [xone-javascript-patterns-d-createobject.md §2.7](xone-javascript-patterns-d-createobject.md#27-filemanager).
...
| `putExtra(name, value)` | String + Object | `AndroidIntent` | Extra genérico (auto-detecta tipo). |
...
| `setX(v)` / `setY(v)` / `setZ(v)` | Translacion absoluta (no existe `setXY`). |
...
- **`Socket`** — Cliente socket TCP/IP en bruto. Métodos disponibles: `setProtocol`, `setAddress`, `setPort`, `setTimeout`, `connect`, `send`, `receive`, `receiveAll`, `disconnect`.
...
Métodos disponibles: `getDeviceId()`, `getLog()`, `sendLog()`, `sendDatabase()`, `sendReplicaDebugDatabase()`, `sendReplicaFilesDatabase()`.
...
wear.send("/path/notificacion"   // ruta literal del path (sin tildes), "Pedido listo");   // método se llama send, no sendMessage
```
...
Impresión en impresoras Bluetooth, USB o de red. Soporta ESC/POS y PDF.
...
// pdf.newPage();    // para página nueva (NO addPage)
...
Métodos `@ScriptAllowed` (57): hashes (`md5`, `sha1`, `sha224`, `sha256`, `sha384`, `sha512`), codificacion (`toBase64`/`fromBase64`, `toBase58`, `toBase45`, `toBase32`), compresion (`inflate`, `deflate`), keystore (`getCertificate`, `getRsaPublicKey`/`getRsaPrivateKey`, `getEcPublicKey`/`getEcPrivateKey`, `installKeyPairOnKeyStore`, etc.), COSE/CBOR (`decodeCose`/`decodeCbor`/`encodeCbor`/`validateCose`), random (`getRandomString`, `getRandomInt`, `getRandomDouble`, `getNewUuid`), criptografia (`hash`, `sign`, `encrypt`, `decrypt`, `derivePassword`, `generateKeyPair`, `generateAesKey`/`installAesKey`/`getAesKey`), JWT (`isJwtSignatureValid`), DB (`getDatabaseKey`), checksum (`getChecksum`).
...
| Método | Descripción |
|---|---|
| `isRunningInMdm(...)` → `boolean` | `true` si la app esta gestionada por MDM. |
| `isPasswordSecured()` → `boolean` | Dispositivo con PIN/password configurado. |
| `getBrightness()` → `double` | Brillo actual en escala **0 - 100** (no 0-1). |
| `setBrightness(value)` → `boolean` | Cambia el brillo (valor 0-100, se clampa al rango). |
...
| `logAnalyticsEvent({eventTag, ...})` → `boolean` | Registra un evento Firebase Analytics. |
| `showAppSettingsWindow()` → `SystemSettings` | Abre los ajustes del sistema para esta app (encadenable). |
| `garbageCollect()` → `SystemSettings` | Fuerza recolección de basura (encadenable). |
...
IA generativa **local** (LLM on-device): ejecuta modelos de lenguaje dentro del dispositivo, sin servidor. Singleton global. API extensa (descarga de modelos, carga, generación, chat con streaming, herramientas, skills, multimodal imagen/audio). Documentación completa en su propio archivo: [xone-javascript-ai.md](xone-javascript-ai.md).
