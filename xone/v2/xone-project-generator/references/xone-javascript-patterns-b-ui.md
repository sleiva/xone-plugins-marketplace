...
**Escanear un documento en papel** — `ui.scanDocument(params)`. Abre el escáner de documentos del sistema: encuadre guiado, detección de bordes, recorte y enderezado automáticos, reencuadre, filtros y varias páginas. Asíncrono, el resultado llega por callback.

| Parámetro | Tipo | Default | Descripción |
|-----------|------|---------|-------------|
| `mode` | string | `"base"` | `"base"` (recorte, rotación y reencuadre), `"baseWithFilters"` (añade filtros de imagen), `"full"` (añade limpieza automática: dedos, manchas). Otro valor da error |
| `pageLimit` | number | `1` | Máximo de páginas por sesión |
| `allowGallery` | boolean | `false` | Permite importar de la galería en lugar de capturar |
| `outputJpg` | boolean | `true` | Genera un JPG por página |
| `outputPdf` | boolean | `false` | Genera además un PDF con todas las páginas |
| `onSuccess` | function | — | Recibe un array con los nombres de fichero generados |
| `onError` | function | — | **Obligatorio**. Recibe el mensaje de error |
| `onCancelled` | function | — | **Obligatorio**. El usuario canceló el escaneo |

Al menos uno de `outputJpg` / `outputPdf` debe quedar activo, o la llamada falla. Los ficheros se escriben en la carpeta de ficheros de la app (`appData.getFilesPath()`) con prefijo `scan_`; el array trae **solo el nombre**, no la ruta: primero los JPG en orden de página y al final el PDF, si se pidió. El escáner lo aporta Google Play Services (módulo descargable): sin servicios de Google la llamada termina en `onError`.
...
ui.scanDocument({
    ...
    onSuccess   : function(aFiles) {
        self.MAP_DOCUMENTO = aFiles[0];   // primera página en un campo de imagen
        self.save();
    },
    onError     : function(sMessage) { ui.showToast("Error al escanear: " + sMessage); },
    onCancelled : function() { ui.showToast("Escaneo cancelado"); }
});
```

**OCR de una imagen** — `ui.recognizeText(params)`. Reconoce el texto (alfabeto latino) de una imagen del dispositivo. Asíncrono. Encaja detrás de `scanDocument` para digitalizar un papel y leer su contenido.
...
| `onError` | function | — | **Obligatorio**. Mensaje de error |
| `roi` | objeto | toda la imagen | `{left, top, width, height}`; valores ≤ 1 son fracción del tamaño, mayores son píxeles |
| `scale` | number | `1` | Amplía el recorte antes de reconocer |
| `grayscale` | boolean | `false` | Desatura antes de reconocer |
| `detail` | boolean | `false` | Devuelve `{text, lines:[{text, confidence, angle, left, top, width, height}]}` con las líneas ordenadas por posición vertical |

Recortar con `roi` es la palanca principal cuando el texto es pequeño: el reconocedor reescala la imagen internamente, así que cuanto menos sobre en el encuadre, más resolución le queda a cada carácter. No se puede restringir el alfabeto reconocido, así que sobre códigos o matrículas hay que validar el resultado por otra vía (dígito de control, expresión regular, formato conocido).
...
| **recognizeText** | OCR de una imagen del dispositivo (ver 2.1.6). |
...
| **scanDocument** | Escanear un documento en papel con la cámara: JPG y/o PDF (ver 2.1.6). |
...
| **startCamera** / **takePhoto** | Cámara y foto. `ui.startCamera({propName, type: "photo"\|"video"\|"attach", useInternalCamera, motionPhoto, size, width, height, quality, maxDuration, onSuccess, onCancelled})` guarda la captura en un campo sin necesidad de pintar un control de cámara; con `useInternalCamera: true` y `motionPhoto: true` captura una foto en movimiento (JPG con un clip de vídeo embebido). |
