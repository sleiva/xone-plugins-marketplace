...
> - Las medidas aceptan sufijo `p` (pixel en el dispositivo de referencia definido por `resolution-width`/`resolution-height`) o `%` (porcentaje del padre). **NO existe `dp`, `px`, `em`, `rem`** — XOne los ignora o trata como `p` según la versión. `p` ≠ Material `dp`: en 1080×1920 (xxhdpi), Material 56dp ≈ 168p (factor ×3).
