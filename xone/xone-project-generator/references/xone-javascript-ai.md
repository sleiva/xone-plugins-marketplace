# Objeto `ai` — IA Generativa Local (LLM on-device)

El objeto global `ai` ejecuta modelos de lenguaje (LLM) **dentro del dispositivo**, sin enviar datos a ningún servidor. La inferencia es 100% local: solo se necesita red para **descargar** el modelo una vez. Es un singleton global — se accede directamente como `ai`, sin `new` ni `createObject()`.

> **Disponibilidad:** el objeto `ai` solo existe en compilaciones del framework que incluyan el módulo de IA embebida. En `ai.isLoaded()` o cualquier llamada fallará si la app no se compiló con esa opción.

Modelos: ficheros en formato `.litertlm` (p. ej. los publicados por la comunidad `litert-community` en HuggingFace). Hay modelos de **solo texto** y **multimodales** (texto + imagen + audio), como la familia **Gemma**.

## Tabla de Contenidos

1. [Flujo típico](#1-flujo-tipico)
2. [Descargar un modelo (`downloadModel`)](#2-descargar-un-modelo-downloadmodel)
3. [Comprobar el dispositivo (`canLoadModel`)](#3-comprobar-el-dispositivo-canloadmodel)
4. [Inspeccionar el modelo (`getModelInfo`)](#4-inspeccionar-el-modelo-getmodelinfo)
5. [Cargar y descargar de memoria (`loadModel` / `unload` / `isLoaded`)](#5-cargar-y-descargar-de-memoria)
6. [Generación sin historial (`generate`)](#6-generacion-sin-historial-generate)
7. [Chat multi-turno con streaming (`chat`)](#7-chat-multi-turno-con-streaming-chat)
8. [Cancelar y reiniciar (`cancel` / `clearChat`)](#8-cancelar-y-reiniciar)
9. [Herramientas / function calling (`tools`)](#9-herramientas--function-calling-tools)
10. [Skills automáticas (`loadSkills`)](#10-skills-automaticas-loadskills)
11. [Multimedia: formatos soportados](#11-multimedia-formatos-soportados)
12. [Parámetros recomendados (Gemma 4)](#12-parametros-recomendados-gemma-4)
13. [Buenas prácticas y problemas comunes](#13-buenas-practicas-y-problemas-comunes)
14. [Referencia rápida de métodos](#14-referencia-rapida-de-metodos)
...
## 14. Referencia rápida de métodos

| Método | Bloqueante | Descripción |
|---|---|---|
| `ai.downloadModel({...})` | No (callbacks) | Descarga un modelo de HuggingFace. |
| `ai.canLoadModel([{path, memoryFactor}])` | Sí | `true` si el dispositivo puede con el modelo. |
| `ai.getModelInfo(file)` | Sí (instantáneo) | Parámetros del modelo sin cargarlo. |
| `ai.loadModel({...})` | Sí / No con callbacks | Carga el modelo en memoria. No bloquea si se pasan `onModelLoaded` + `onModelLoadError`. |
| `ai.isLoaded()` | Sí | `true` si hay modelo cargado. |
| `ai.unload()` | Sí | Libera el modelo de memoria. |
| `ai.generate({...})` | **Sí** | Respuesta única, sin historial. Devuelve `String`. No en hilo de UI. |
| `ai.chat({...})` | No (callbacks) | Conversación con historial y streaming. |
| `ai.cancel()` | Sí | Cancela el `chat` en curso. |
| `ai.clearChat()` | Sí | Reinicia el historial de la conversación. |
| `tools: [{jsonDescriptorPath, callback}]` | — | Parámetro de `generate`/`chat`: herramientas (function calling). Ver §9. |
| `ai.loadSkills(dir)` | Sí | Carga skills (`.md`) de una carpeta. |
| `ai.removeSkill(name)` | Sí | Elimina una skill. |
| `ai.clearSkills()` | Sí | Elimina todas las skills. |
